...
export const PracticeEngine = {
  ...
  async generateSession(
    ...
    const targetCourse = courses.find((c: any) => c.id === targetCourseId) || courses[0] || null;
    ...
    const targetLecture =
      safeLectures.find((l) => l.id === targetLectureId) ||
      safeLectures[0] ||
  ...
  // ─────────────────────────────────────────────────────────────────────────
  async regenerateQuestions(session: PracticeSession): Promise<PracticeSession> {
    console.log('[PRACTICE ENGINE] Regenerating questions for:', session.topic);
  ...
  async _fetchQuestions({
    ...
    try {
      const payload = {
        ...
        topic: targetLecture.title || targetLecture.topic || 'General Concepts',
        ...
        resources: resources.map((r: any) => ({
          ...
          summary: r.summary || `Focus on the topic: ${targetLecture.title}`,
        })),
      };
      ...
      if (!response.ok) {
        ...
        console.error('[AI Generation failed]', response.status, response.statusText, errorText);
        return [];
      }
      ...
      console.error('[PRACTICE ENGINE] Failed to connect to AI generator:', e);
      return [];
    }
  },
};